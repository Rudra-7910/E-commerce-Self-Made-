...
function Login() {
    ...
    const loginUser = async(data)=>{
        const response = await axios.post(`${import.meta.env.PROD ? 'https://e-commerce-self-made-production.up.railway.app' : 'http://localhost:3000'}/api/user/login`, data)
        return response.data
    }
    const verifyUser = async(data)=>{
        const response = await axios.post(`${import.meta.env.PROD ? 'https://e-commerce-self-made-production.up.railway.app' : 'http://localhost:3000'}/api/user/verify`, data)
        return response.data
    }
    ...
    const verifyMutation = useMutation({
        ...
        onSuccess: (data) => {
            ...
            if(data.token) {
                login(data.token);
                // AuthRoute will automatically redirect to "/" after state update
            }
        },
        onError: (error) => {
            console.log(error.response?.data);
  ...
  return (
    ...
    <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col border-2 p-10 rounded-2xl gap-4 bg-white w-96 shadow-lg'>
        <h2 className='text-2xl font-bold text-center text-slate-700 mb-4'>Login</h2>
        {apiError && (
            <div className='bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-2xl shadow-xs mb-5 '>
        ...
        >
            {loginMutation.isPending || verifyMutation.isPending ? "Please wait..." : (showOtp ? "Verify & Login" : "Send OTP")}
        </button>
